...
function ChatApp() {
   ...
   * Depending on whether it's a user message or a server message, it's displayed differently (you on the right, else Bot on the left).
  ...
  const renderMessageAsMarkdown = (msg) => {
    ...
    const displayMessage = isUserMessage ? `<b>You:</b><br />${msg.substring(5)}` : `<b>Bot:</b><br />${msg}`;
